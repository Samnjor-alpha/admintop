...
if(!isset($_GET['categ'])){
    ...
    $market=mysqli_query($conn,"Select * from ftmarkets where mrkt_id='$categid'");

    $row_htmrkt=$market->fetch_assoc();
    $mrktcateg=$row_htmrkt['mrktcateg_id'];

    if (isset($_POST['upftmrkt']))    {
        ...
        if (empty($_POST['mrktdesc']) || empty($_POST['mrkt']) || empty($_POST['mrkt_categ'])) {
            ...
            $sql_e = "SELECT * FROM ftmarkets WHERE mrkt_name ='$mrktname' and not mrkt_id='$categid'";
            ...
            if (mysqli_num_rows($res_e)>0){
                ...
                if(empty($error)) {
                    ...
                    $sql = "update ftmarkets SET mrkt_name ='$mrktname',mrkt_desc='$mrktdesc' ,mrktcateg_id='$mrkt_categid' where mrkt_id='$categid'";
