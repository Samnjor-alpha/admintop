...
function check1or2($gameid,$tipid,$mrktid){
    global $conn;
    ...
    if (mysqli_num_rows($takescores)>0) {
        ...
        $status='';
        $statusclass="";
        if ($hscore>$ascore || $ascore>$hscore) {
            $status='1';
...
$homeawaytips=mysqli_query($conn, "SELECT * FROM  predictions	INNER JOIN fixtures ON fixtures.fix_code = predictions.game_id
