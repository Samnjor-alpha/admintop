...
function check_ggn($gameid,$tipid,$mrktid){
    global $conn;
    ...
    if (mysqli_num_rows($takescores)>0) {
        ...
        if ($hscore<1 || $ascore<1) {
            $status='1';
            $updateresult=mysqli_query($conn,"update predictions set status='$status' where id='$tipid' and tip_id='$mrktid'");
            $statusclass="won";
        }else{
            $status='2';
            $statusclass="Lost";
